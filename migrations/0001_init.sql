...
-- The single server identity row. `oprf_seed` is the OPAQUE server
-- secret scalar generated once at first deploy by
-- `scripts/seed-opaque-identity.mjs`. It must be generated by the
-- OPAQUE suite, not arbitrary random bytes; rotating it invalidates
-- every existing OPAQUE registration record, so it's append-only by
-- convention.
