...
async function main() {
	...
	// NOTE on /_app/env.js: it carries $env/dynamic/public, including the
	// per-release PUBLIC_BUNDLE_HASH the in-page integrity verifier compares
	// the running bundle against, and Cloudflare edge-caches the .js URL by
	// default (~4h) — serving the PREVIOUS release's hash after a deploy and
	// tripping the integrity gate. A `_headers` rule does NOT control it:
	// adapter-cloudflare serves env.js from its built-in env handler, which
	// bypasses both this static `_headers` file and the SvelteKit `handle`
	// hook (verified: env.js carries none of the SSR security headers). The
	// edge cache for /_app/env.js is therefore controlled OUT OF BAND by a
	// Cloudflare zone Cache Rule (URI Path = /_app/env.js → Bypass cache).
