...
async function main() {
	...
	const HEADERS_OVERRIDE =
		...
		// /_app/env.js carries $env/dynamic/public — including the
		// per-release PUBLIC_BUNDLE_HASH the in-page integrity verifier
		// compares against. It MUST NOT be edge-cached: a 4h CDN cache
		// (observed cf-cache-status:HIT, max-age=14400) keeps serving the
		// PREVIOUS release's hash after a new deploy, so the running
		// bundle hash no longer matches the env hash and the integrity
		// gate (and the unlock screen) reject the vault until the cache
		// expires. no-store keeps the runtime env in lockstep with the
		// deployed bundle.
		'/_app/env.js\n' +
		'  ! Cache-Control\n' +
		'  Cache-Control: no-store\n';
